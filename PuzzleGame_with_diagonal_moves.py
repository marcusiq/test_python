...
started = False
total_moves = 0
stack_of_moves = []  # Records the number of each tile that is moved and where it was placed. Each move is thus a tuple.
...
def start():
    global valid
    check_button.configure(state=DISABLED) # Can't change once the game has started.
    print "The value of the Checkbutton is:", var.get()
    if var.get() == 0:
        print "Case 0"
        valid = {0: [1, 4], 1: [0, 2, 5], 2: [1, 3, 6], 3: [2, 7], 4: [0, 5, 8],
                 5: [1, 4, 6, 9], 6: [2, 5, 7, 10], 7: [3, 6, 11], 8: [4, 9], 9: [5, 8, 10], 10: [6, 9, 11],
                 11: [7, 10]}
    else:
        print "Case 1" # we will allow diagonal moves.
        valid = {0: [1, 4, 5], 1: [0, 2, 4, 5, 6], 2: [1, 3, 5, 6, 7], 3: [2, 6, 7], 4: [0, 1, 5, 8, 9],
                 5: [0, 1, 2, 4, 6, 8, 9, 10], 6: [1, 2, 3, 5, 7, 9, 10, 11], 7: [2, 3, 6, 10, 11], 8: [4, 5, 9],
                 9: [4, 5, 6, 8, 10], 10: [5, 6, 7, 9, 11], 11: [6, 7, 10]}
    print "The allowed moves are:",  valid
...
def mix_up(n):
    ...
    if n < 6:                                                  # Makes 6 "random" moves
        ...
        status_label.configure(text="Status: Puzzle Challenge in Progress. Good luck!")
...
def move_tile(t, n):
    ...
    if n == 0:
        ...
        sw = 3; ne = -3  # Added to help with the diagonal moves in the last problem.
        ...
        if direction == sw:         dx = -25; dy = +20  # Code for a possible diagonal move!
        if direction == nw:         dx = -25; dy = -20  # Code for a possible diagonal move!
        if direction == ne:         dx = +25; dy = -20  # Code for a possible diagonal move!
...
def win():
    ...
    status_label.configure(text="Status: YOU WON! The golden dragon will grant you one wish.")
    for tile in tiles:
...
# This first dictionary only allows moves that are up, down, to the left or right.
valid = {0: [1, 4], 1: [0, 2, 5], 2: [1, 3, 6], 3: [2, 7], 4: [0, 5, 8],
         5: [1, 4, 6, 9], 6: [2, 5, 7, 10], 7: [3, 6, 11], 8: [4, 9], 9:[5, 8, 10], 10:[6, 9, 11],  11:[7, 10]}

# This second dictionary allows all the previous moves, and the four diagonal moves.
# Comment it out to disallow diagonal moves.
...
# New code added!
# A check button is added to allow diagonal moves, or just the usual up/dpwn or left/right options
var = IntVar()
check_button = Checkbutton(info_frame, text="Allow Diagonal Moves" , variable=var)
check_button.grid(row=0, column=5, sticky=E)
