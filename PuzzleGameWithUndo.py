...
random.seed(0)  # Make the random moves deterministic.


# featuires simpler logic for the solver by adding an undo button
...
def undo():
    print "The UNDO button was pressed."
    stack_of_moves.pop()  # pop off the current free space
    print "The stack of moves is now: ", stack_of_moves

    tile_number = stack_of_moves[-1]
    move_tile(tile_number, 0)


def solve():
    delay = 700  # in milliseconds
    number_of_moves = len(stack_of_moves) - 1
    for k in range(number_of_moves):
        puzzle_root.after(k * delay, undo)
...
def solve():
    ...
    while stack_of_moves:  # True until the list is empty.
        ...
        puzzle_root.after( k*delay, move_tile, tile_number, 0 )
...
started = False
total_moves = 0
stack_of_moves = [8]  # Records the number of each tile that is moved and where it was placed. Each move is this a tuple.
...
undo_button = Button(puzzle_root, text=" UNDO ", command=undo)
